...
class AnnullaChiamataScreen extends StatefulWidget {
  const AnnullaChiamataScreen({Key? key}) : super(key: key);

  @override
  State<AnnullaChiamataScreen> createState() => _AnnullaChiamataScreenState();
}

class _AnnullaChiamataScreenState extends State<AnnullaChiamataScreen> {
    ...
    return await showDialog<Map<String, DateTime>>(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setStateDialog) {
          return AlertDialog(
            title: Text('Periodo da annullare per ${dipendente.nome}'),
            content: Column(
  ...
  Future<void> _inviaAnnullamento() async {
...
// Validazione date in base alla modalità
      ...
      try {
        ...
        // Crea lista di chiamate annullate per tutti i dipendenti
        List<Chiamata> chiamateAnnullate = [];
        for (var dipendente in dipendentiSelezionati) {
          ...
          chiamateAnnullate.add(Chiamata(
        ...
        if (tipoInvio == 'app') {
          ...
          await AppEmailService.sendEmailViaApp(
            ...
            subject: dipendentiSelezionati.length > 1
                ? 'ANNULLAMENTO Comunicazione Intermittenti - ${dipendentiSelezionati.length} dipendenti'
                : 'ANNULLAMENTO Comunicazione Intermittenti - ${dipendentiSelezionati[0].nome}',
            body: dipendentiSelezionati.length > 1
                ? 'In allegato l\'annullamento delle comunicazioni obbligatorie per ${dipendentiSelezionati.length} dipendenti.\n\n'
                '${usaStessaData ? "Periodo annullato comune: ${DateFormat('dd/MM/yyyy').format(dataInizioComune!)} - ${DateFormat('dd/MM/yyyy').format(dataFineComune!)}" : "Periodi annullati specifici per ciascun dipendente"}'
                : 'In allegato l\'annullamento della comunicazione obbligatoria per il dipendente ${dipendentiSelezionati[0].nome}.\n\n'
                'Periodo annullato: ${DateFormat('dd/MM/yyyy').format(usaStessaData ? dataInizioComune! : dateInizio[dipendentiSelezionati[0].id!]!)} - ${DateFormat('dd/MM/yyyy').format(usaStessaData ? dataFineComune! : dateFine[dipendentiSelezionati[0].id!]!)}',
          ...
          final salvare = await showDialog<bool>(
            ...
            builder: (context) => AlertDialog(
              ...
              content: const Text('Hai inviato l\'annullamento? Vuoi salvarlo nello storico?'),
              actions: [
                TextButton(
              ...
              builder: (context) => AlertDialog(
                content: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 20),
                    const Text(
                      'Annullamento inviato con successo!',
                      style: TextStyle(
                        fontSize: 18,
                        fontWeight: FontWeight.bold,
                      ),
                    ...
                    Text(
                      dipendentiSelezionati.length > 1
                          ? 'Annullamento per ${dipendentiSelezionati.length} dipendenti inviato'
                          : 'Annullamento inviato correttamente',
                      style: TextStyle(
                        fontSize: 14,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Annulla Chiamata'),
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: const Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'Seleziona i dipendenti e il periodo da annullare',
              ...
              CheckboxListTile(
                title: const Text(
                  'Usa stesso periodo per tutti i dipendenti',
                  style: TextStyle(fontWeight: FontWeight.bold),
                ),
                subtitle: const Text(
                  'Se disattivato, potrai scegliere periodi diversi per ogni dipendente',
              ...
              // Pulsante "Seleziona tutti" - appare solo se usa stessa data
              ...
              // Mostra dettagli di tutti i dipendenti selezionati
              ...
              if (usaStessaData) ...[
                const SizedBox(height: 20),
                ListTile(
                  title: const Text('Data Inizio Periodo da Annullare (per tutti) *'),
                  subtitle: Text(
                ...
                ListTile(
                  title: const Text('Data Fine Periodo da Annullare (per tutti) *'),
                  subtitle: Text(
                    dataFineComune != null
              ...
              ElevatedButton(
                onPressed: isSending ? null : _inviaAnnullamento,
                style: ElevatedButton.styleFrom(
                  backgroundColor: Colors.red,
